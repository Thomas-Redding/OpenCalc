...
@implementation MathNaturalLogarithm

- (MathNaturalLogarithm*) init {
    self.name = @"ln";
    return self;
}
    ...
    if([input[0] objectType] == MATHNUMBER) {
        ...
        if(imag == 0) {
            ...
            if(real > 0) {
                ...
                [answer setDouble:log(real)];
                return answer;
            }
            else if(real < 0) {
                // negative
                MathNumber *answer = [[MathNumber alloc] init];
                [answer setDouble:log(-1*real)];
                [answer setDouble:1 newValue:3.14159265358979323846];
                return answer;
            }
            else {
                // zero
            ...
            if(imag > 0) {
                // positive
                [answer setDouble:log(imag)];
                [answer setDouble:1 newValue:3.14159265358979323846];
                return answer;
            }
            else {
                // negative
                [answer setDouble:log(-1*imag)];
                [answer setDouble:1 newValue:-3.14159265358979323846];
                return answer;
            }
        }
